...
TopCorGenes <- function(CorValueResaults, # input list from CorValueByGenes_V3
  ...
  TopNcellTypesValue <- lapply(cellCor, function(x){
    cellTypeNames  <- names(head(sort(x, decreasing = T),cellTypeNum))
    x[cellTypeNames]
  })
  ...
  for(i in 1:length(CorValueResaults)){
    TopNcellTypes_df[[i]] <- CorValueResaults[[i]][,names(TopNcellTypesValue[[i]])]
  }
  ...
  TopCorGenes_list <- list(TopCorGenes_mx = TopCorGenes_mx, 
                           TopNcellTypesValue =TopNcellTypesValue)
  return(TopCorGenes_list)
}
