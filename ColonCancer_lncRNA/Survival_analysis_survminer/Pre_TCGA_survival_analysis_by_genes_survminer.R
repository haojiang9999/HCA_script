...
rownames(TCGA.RSEM.gene.tpm) <- gsub("\\..*","",genelist)
TCGA.RSEM.gene.tpm[1:5,1:5]
## 2. Extract target gene expression data
geneName <- "ENSG00000234851"
geneTPM <- TCGA.RSEM.gene.tpm[geneName,]
nonZeroPercent <- sum(geneTPM != 0 ) / length(geneTPM)
# Too many zeros may affect resaults 
# if nonZeroPercent < 0.5 shall we remove all zero samples??
### !!!!!!!!!!!!
### I shuold add another split part according to nonZeroPercent
# order samples by gene expression 
...
require("survminer")
splots <- list()
splots[[1]] <- ggsurvplot(fit.firstQ, pval = T,
...
splots[[4]] <- ggsurvplot(fit.nonZeroQ, pval = T,
                          ...
                          legend.title = paste0("nonZeroPercent =", nonZeroPercent,
                                                " N = ", sampleNumber))
## 6. Arrange multiple ggsurvplots and print the output
