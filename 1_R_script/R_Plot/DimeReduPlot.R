...
DimeReduPlot <- function(mx , color, tiltle = "Something", print = T, ...){
  ...
  require(ggplot2)
  # mx = scale(Cor.Res.CV1500$Cor.merged)
  #### PCA plot
  mx[is.na(mx)] <- 0
  pca <- prcomp(t(mx)); 
  pca_out <- as.data.frame(pca$x)
  p1<-ggplot(pca_out,aes(x=PC1,y=PC2,color = color ))+geom_point() + labs(title = tiltle, subtitle = "PCA plot")
  
  ### t-SNE plot
  require(Rtsne)
  #t(test_for_clust_power)
  set.seed(123)
  TSNE <- Rtsne(t(mx), dims = 2, initial_dims = 50,...)
  ...
  print(p3)}else{
    ...
    ggexport(res.arr, filename = paste0(tiltle, "_PCA_tSNE_UMAP_Plots.pdf"),width = 15, height = 35)
  }
}
