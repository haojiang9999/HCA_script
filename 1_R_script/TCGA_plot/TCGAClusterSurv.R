...
TCGAClusterSurv <- function(Input.tb, hclust.res, Col.anno, k = 2, col_anno = c("sampleTypes","histological_type")){
  ...
  Col.anno.cluster <- cbind(Col.anno[,col_anno],as.character(cutree.res))
  ...
  require(survminer)
  require("survival")
  fitOS <- survfit(Surv(OS.time, OS) ~ cutree.res, data = sur.df)
  fitDSS <- survfit(Surv(DSS.time, DSS) ~ cutree.res, data = sur.df)
  fitDFI <- survfit(Surv(DFI.time, DFI) ~ cutree.res, data = sur.df)
  fitPFI <- survfit(Surv(PFI.time, PFI) ~ cutree.res, data = sur.df)
  # Drawing curves
