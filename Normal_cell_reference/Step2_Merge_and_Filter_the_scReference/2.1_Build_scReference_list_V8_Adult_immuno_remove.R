...
scReference.list.V8 <- list(Tang.Adult.colon = Tang.Adult.colon.ref[,-which(names(Tang.Adult.colon.ref) %in% "Adult_Immune")],
                            ...
                            RCA.CRC.nonEpi.ref = RCA.CRC.nonEpi.ref)

### Step1.Find genes had high coefficience-varience
source("/data8t_4/JH/MyJobs/1_R_script/FUN_TopCV.R")
scReference.list.CV8000 <- lapply(scReference.list.V8, function(x){
  TopCV(x, TopN = 8000, MARGIN = 1)
})
scReference.list.CV4500 <- lapply(scReference.list.V8, function(x){
  TopCV(x, TopN = 4500, MARGIN = 1)
})
scReference.list.CV4000 <- lapply(scReference.list.V8, function(x){
  TopCV(x, TopN = 4000, MARGIN = 1)
})
scReference.list.CV3500 <- lapply(scReference.list.V8, function(x){
...
scReference.list.CV2000 <- lapply(scReference.list.V8, function(x){
...
scReference.log10.CV.V8 <- list(scReference.list.log10.CV8000 = log10.scReference.list.CV.8000,
...
scReference.log10.CV.V8$scReference.list.log10.CV8000$RCA.CRC.nonEpi.ref
